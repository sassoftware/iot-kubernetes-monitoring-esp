...
# ESP Monitoring
if helm3ReleaseExists loki $MON_NS; then
   helm uninstall --namespace $MON_NS loki

   if [ $? != 0 ]; then
     log_warn "Uninstall of loki was not successful. Check output above for details."
   fi

   log_info "Removing Promtail cluster binding role..."
   roles=( promtail )
   for role in "${roles[@]}"
   do
     kubectl delete --ignore-not-found ClusterRoleBinding $role
   done

   log_info "Removing Promtail cluster role..."
   roles=( promtail )
   for role in "${roles[@]}"
   do
     kubectl delete --ignore-not-found ClusterRole $role
   done
fi
# ESP Monitoring
