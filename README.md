...
`ESP_CPU_USAGE` reports on CPU consumption at the ESP window level, whereas `ESP_MEM_USAGE` shows memory info based on type (total, vm, and RSS).

A few other counters are available that reports on the total number of metrics sent to Prometheus as well as the total number of bytes transferred to the Prometheus server.

As a side note, our documentation indicates that ESP uses the `/metrics` endpoint to dump its metrics. In reality, that endpoint is used to gather metrics for the metering server. The ESP server metrics are instead collected using the `/SASESP/metrics` endpoint. 

<img src='images/esp_cpu.png'>

*Figure 2 - ESP CPU Usage*

<img src='/images/esp_memory.png'>

*Figure 3 - ESP Memory Usage*
