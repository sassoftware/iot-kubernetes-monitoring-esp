...
if [ "$AUTOGENERATE_SMTP" == "true" ]; then
    ...
    smtpUserSecret="${SMTP_USER_SECRET:-grafana-smtp-user}"
    ...
    if [ -n "$smtpUserSecret" ] && [ -n "$(kubectl get secret -n "$MON_NS" "$smtpUserSecret" -o name 2> /dev/null)" ]; then
        log_debug "Using secret [$smtpUserSecret] for SMTP user credentials"
...
if [ "$AUTOGENERATE_INGRESS" == "true" ]; then
    ...
    if [ "$INGRESS_USE_SEPARATE_CERTS" == "true" ]; then
        ...
        ingress_tls_secret="v4m-ingress-tls-secret"
    ...
    GRAFANA_FQDN="$(get_app_ingress_fqdn "$GRAFANA_FQDN" "$GRAFANA_PATH")"

    PROMETHEUS_INGRESS_ENABLE="${PROMETHEUS_INGRESS_ENABLE:-false}"
    PROMETHEUS_PATH="${PROMETHEUS_PATH:-prometheus}"
    PROMETHEUS_FQDN="$(get_app_ingress_fqdn "$PROMETHEUS_FQDN" "$PROMETHEUS_PATH")"

    log_debug "ALERTMANAGER_INGRESS_ENABLE [$ALERTMANAGER_INGRESS_ENABLE] ALERTMANAGER_FQDN [$ALERTMANAGER_FQDN] ALERTMANAGER_PATH [$ALERTMANAGER_PATH]"
    log_debug "GRAFANA_INGRESS_ENABLE      [$GRAFANA_INGRESS_ENABLE]      GRAFANA_FQDN      [$GRAFANA_FQDN]      GRAFANA_PATH      [$GRAFANA_PATH]"
    log_debug "PROMETHEUS_INGRESS_ENABLE   [$PROMETHEUS_INGRESS_ENABLE]   PROMETHEUS_FQDN   [$PROMETHEUS_FQDN]   PROMETHEUS_PATH   [$PROMETHEUS_PATH]"

    export ALERTMANAGER_INGRESS_ENABLE ALERTMANAGER_FQDN ALERTMANAGER_PATH
    export GRAFANA_INGRESS_ENABLE GRAFANA_FQDN GRAFANA_PATH
    export PROMETHEUS_INGRESS_ENABLE PROMETHEUS_FQDN PROMETHEUS_PATH

    if [ "$INGRESS_TYPE" == "ingress-nginx" ]; then
        ...
        yq -i '.alertmanager.ingress.enabled=env(ALERTMANAGER_INGRESS_ENABLE)' "$autogeneratedYAMLFile"
        yq -i '.grafana.ingress.enabled=env(GRAFANA_INGRESS_ENABLE)' "$autogeneratedYAMLFile"
        yq -i '.prometheus.ingress.enabled=env(PROMETHEUS_INGRESS_ENABLE)' "$autogeneratedYAMLFile"

        if [ "$INGRESS_USE_SEPARATE_CERTS" == "true" ]; then
            yq -i '.alertmanger.ingress.tls.[0].secretName.[0]="v4m-ingress-tls-secret"' "$autogeneratedYAMLFile"
            yq -i '.grafana.ingress.tls.[0].secretName.[0]="v4m-ingress-tls-secret"' "$autogeneratedYAMLFile"
            yq -i '.prometheus.ingress.tls.[0].secretName.[0]="v4m-ingress-tls-secret"' "$autogeneratedYAMLFile"
        fi
        ...
        if [ "$ROUTING" == "host" ]; then
            ...
            rooturl="https://$GRAFANA_FQDN/$GRAFANA_PATH" yq -i '.grafana."grafana.ini".server.root_url=env(rooturl)' "$autogeneratedYAMLFile"
            ...
            exturl="https://$PROMETHEUS_FQDN/$PROMETHEUS_PATH" yq -i '.prometheus.prometheusSpec.externalUrl=env(exturl)' "$autogeneratedYAMLFile"
        ...
        ## tested with sample version: 0.2.2
        ingressSampleFile="samples/contour/${ROUTING}-based/monitoring/user-values-prom-operator.yaml"

        #intialized the yaml file w/appropriate ingress sample, uncomment blocks for Alertmanager and Prometnheus
        sed s/##//g "$ingressSampleFile" > "$autogeneratedYAMLFile"
        ...
        if [ "$ALERTMANAGER_INGRESS_ENABLE" == "true" ]; then
            ...
            yq -i 'del (.alertmanager)' "$autogeneratedYAMLFile"
        fi
        ...
        if [ "$GRAFANA_INGRESS_ENABLE" == "true" ]; then
            ...
            yq -i 'del (.grafana)' "$autogeneratedYAMLFile"
        fi
        ...
        if [ "$PROMETHEUS_INGRESS_ENABLE" == "true" ]; then
            ...
            yq -i 'del (.prometheus)' "$autogeneratedYAMLFile"
        fi

        if [ "$INGRESS_CREATE_ROOT_PROXY" == "true" ]; then
            #NOTE: Calling create_root_httpproxy *function*,
...
if [ "${GRAFANA_AUTHENTICATION^^}" == "LDAP" ] || [ "${GRAFANA_AUTHENTICATION^^}" == "OAUTH" ]; then
   ...
   if [ "$ESP_GRAFANA_PLUGIN_VERSION" != "null" ]; then
      if [ "${GRAFANA_AUTHENTICATION^^}" == "OAUTH" ]; then
         $USER_DIR/monitoring/grafana/esp-plugin/grafana-esp-plugin-main/install/configure-grafana.sh "${ESP_NAMESPACE}" "${MON_NS}" "${ESP_GRAFANA_PLUGIN_VERSION}"
      else
   ...
   if [ "${GRAFANA_AUTHENTICATION^^}" == "LDAP" ]; then
   ...
   if [ "${GRAFANA_AUTHENTICATION^^}" == "OAUTH" ]; then
      AUTH_DIR="$(realpath "${USER_DIR}")/monitoring/grafana/esp-plugin/grafana-esp-plugin-main/install/manifests"
      kubectl apply -n $MON_NS -f $AUTH_DIR/v4m-grafana-config.yaml
      kubectl patch deployment v4m-grafana -n $MON_NS --patch-file $AUTH_DIR/v4m-grafana-patch.yaml
      ...
      log_verbose "USER_DIR = $USER_DIR"
      log_verbose "GRAFANA_AUTH_PROVIDER = $GRAFANA_AUTH_PROVIDER"
      $USER_DIR/monitoring/grafana/esp-plugin/grafana-esp-plugin-main/install/register-oauth-client-"${GRAFANA_AUTH_PROVIDER}".sh "${ESP_NAMESPACE}"
   fi
